...
/* Subject Alternative Name Values (SAN) [what a bad interface] */
  ...
  for (int i = 0, n = 0; i < num; ++i) {
    ...
    if (ext->type == GEN_DNS) {
      if (OBJ_sn2nid((const char*)ext->d.ia5) != V_ASN1_IA5STRING) continue;
        yname = (char *)ASN1_STRING_get0_data(ext->d.ia5);
        if (n == 0) {
          if (!env_str("SSL_CLIENT_SAN_DNS_0=",yname)) return 0;
        }  else n = 1;
        if (n == 1)
          if (!env_str("SSL_CLIENT_SAN_DNS_1=",yname)) return 0;
    }
    if (ext->type == GEN_EMAIL) {
      n = 0;
      if (OBJ_sn2nid((const char*)ext->d.ia5) != V_ASN1_IA5STRING) continue;
        yname = (char *)ASN1_STRING_get0_data(ext->d.ia5);
        if (n == 0) {
          if (!env_str("SSL_CLIENT_SAN_Email_0=",yname)) return 0;
        } else n = 1;
        if (n == 1)
          if (!env_str("SSL_CLIENT_SAN_Email_1=",yname)) return 0;
    }
  }
  ...
  for (int i = 0, n = 0; i < num; ++i) {
    ...
    if (ext->type == GEN_DNS) {
      if (OBJ_sn2nid((const char*)ext->d.ia5) != V_ASN1_IA5STRING) continue;
        yname = (char *)ASN1_STRING_get0_data(ext->d.ia5);
        if (n == 0) {
          if (!env_str("SSL_SERVER_SAN_DNS_0=",yname)) return 0;
        } else n = 1;
        if (n == 1)
          if (!env_str("SSL_SERVER_SAN_DNS_1=",yname)) return 0;
    }
    if (ext->type == GEN_EMAIL) {
      n = 0;
      if (OBJ_sn2nid((const char*)ext->d.ia5) != V_ASN1_IA5STRING) continue;
        yname = (char *)ASN1_STRING_get0_data(ext->d.ia5);
        if (n == 0) {
          if (!env_str("SSL_SERVER_SAN_Email_0=",yname)) return 0;
        } else n = 1;
        if (n == 1)
          if (!env_str("SSL_SERVER_SAN_Email_1=",yname)) return 0;
    }
  }
...
int ssl_client_env(SSL *ssl,stralloc *sa) 
{
  ...
  if (!ssl_server_vars(0,SSL_get_peer_cert_chain(ssl))) return 0;
  return 1;
}
