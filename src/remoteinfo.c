...
static ssize_t mywrite(int fd,char *buf,size_t len)
{
  iopause_fd x;
  int r;
...
static ssize_t myread(int fd,char *buf,size_t len)
{
  iopause_fd x;
  int r;
...
static int doit(stralloc *out,int s,char ipremote[16],uint16 portremote,
                char iplocal[16],uint16 portlocal,unsigned int timeout,uint32 netif)
{
  buffer b;
  char bspace[128];
...
int remoteinfo(stralloc *out,char ipremote[16],uint16 portremote,
               char iplocal[16],uint16 portlocal,unsigned int timeout,uint32 netif)
{
  int s;
  int r;
